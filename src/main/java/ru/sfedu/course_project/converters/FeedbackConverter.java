...
public class FeedbackConverter extends AbstractCsvConverter {
    public static Logger log = LogManager.getLogger(FeedbackConverter.class);
    ...
    public Object convertToRead(String s) throws CsvDataTypeMismatchException, CsvConstraintViolationException {
        ...
        for (int i = 0; i < 2; i++) {
            ...
            if (instance == "Comment") {
                feedback = new Comment(UUID.randomUUID(), Role.valueOf((String) values.get("role")), (String) values.get("text"));
            } else if (instance == "Assessment") {
                feedback = new Assessment(UUID.randomUUID(), Role.valueOf((String) values.get("role")), (Mark) values.get("mark"));
            }
