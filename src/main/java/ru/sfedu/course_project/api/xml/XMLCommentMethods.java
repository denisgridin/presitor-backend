...
public class XMLCommentMethods {

    public XMLCommentMethods() {}

    private static final Logger log = LogManager.getLogger(XMLCommentMethods.class);
    ...
    public static Result getPresentationComments (HashMap args) {
        try {
            ...
            Collection<Comment> comments = (ArrayList<Comment>) XMLCommonMethods.getCollection(comment).orElse(new ArrayList());
    ...
    public static Status writeCommentsCollection (Comment comment) {
        try {
            ...
            ArrayList comments = (ArrayList) XMLCommonMethods.getCollection(CollectionType.comment).orElse(new ArrayList());
            comments.add(comment);
            Status status = XMLCommonMethods.writeCollection(comments, Comment.class, CollectionType.comment);
            return status;
        } catch (RuntimeException e) {
            log.error(e);
            return Status.error;
    ...
    public static Result editPresentationComment (HashMap arguments) {
        try {
            ...
            Optional<Comment> optionalComment = XMLCommonMethods.getInstanceExistenceByField(comment, Comment.class, ConstantsField.ID, (String) arguments.get(ConstantsField.ID));
            ...
            Result resultUpdate = XMLCommonMethods.updateRecordInCollection(Comment.class, CollectionType.comment, comment, comment.getId());
            return resultUpdate;
        } catch (RuntimeException e) {
            log.error(e);
    ...
    public static Result removePresentationComment (HashMap arguments) {
        try {
            ...
            Comment comment = optionalComment.get();
            Status status = XMLCommonMethods.removeRecordById(CollectionType.comment, Comment.class, comment.getId());
